...
var mouvement_speed = 60.0
var hp = 80
var maxhp = 80
var last_mouvement = Vector2.UP 
var time = 0
...
@onready var itemOption = preload("res://scenes/utility/item_option.tscn")
...
@onready var levelPanel = get_node("%level_up")
...
func movement():
	...
	velocity = mov.normalized()*mouvement_speed
	move_and_slide()
...
func _on_tornado_timer_timeout() -> void:
	tornado_ammo += tornado_baseammo + additional_attack
...
func _on_tornado_attack_timer_timeout() -> void:
	if tornado_ammo > 0:
		...
		tornado_attack.last_mouvement = last_mouvement
...
func spawn_javelin():
	...
	var calc_spawns = (javelin_ammo + additional_attack) - get_javelin_total
	while calc_spawns > 0:
...
func calculate_experiencecap():
	...
	if experience_level < 20:
		exp_cap = experience_level*5   #experience requis jusquau lvl 20
	elif experience_level < 40 :
		exp_cap = 95 * (experience_level-19)*8     #xp requis jusqu'au lvl 40
	else:
...
func upgrade_charactere(upgrade):
	match upgrade:
		"icespear1":
			icespear_level = 1
			icespear_baseammo += 1
...
func get_random_item():
	...
	for i in UpgradeDb.UPGRADES:
		if i in collected_upgrades: #upgrades deja colecter  
			pass
		elif i in upgrade_options: #si l'upgrade est deja une option
			pass
		elif UpgradeDb.UPGRADES[i]["type"] == "item": #prend pas la food
			pass
		elif UpgradeDb.UPGRADES[i]["prerequisite"].size() > 0: #check pour prerequis
			var to_add = true
...
func _on_btn_menu_click_end() -> void:
	get_tree().paused = false
	var _level = get_tree().change_scene_to_file("res://scenes/titl screen/menu.tscn")
